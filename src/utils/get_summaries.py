...
def get_selected(df, select, samp):
    ...
    # Drop duplicates, taking the first genre for a plot as the only genre.
    # Then sample the resultant dataframe to a fraction of the total number of
    # selected plots
    selected = selected.drop_duplicates("wiki_id").sample(frac=samp)
...
def main(args):
    ...
    # Merge the metadata and plots on their Wiki ID. Format genres and remove
    # empty genres
    ...
    for idx in selected.index:
        path = os.path.join(
            args.outdir, str(selected.at[idx, "wiki_id"]) + ".txt"
        )
    ...
    # Create a manifest
    selected = (
        selected.assign(fname=selected["wiki_id"].astype(str) + ".txt")
        .sort_values("fname")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--samp", type=float)
